...
filtered_result = []
def search_result(cars, search_criterias):
# for easier usage search criteria
  year, engine_volume, price = search_criterias
  for car, values in cars.items():
      if values[1] >= year and values[2] >= engine_volume and values[-1] <= price:
          filtered_result.append((car, values))
  # sorting by price and showing first 5 results
  return sorted(filtered_result, key=lambda price:price[1][-1])[:5]

print(search_result(car_data, search_criteria))
